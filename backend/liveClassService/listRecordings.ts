 ...
 * S3_LIVE_CLASS_KEY_REGEX, it will be skipped.
...
function processItem(item: _Object, classes: Record<string, LiveClass>) {
    const matches = S3_LIVE_CLASS_KEY_REGEX.exec(item.Key ?? '');
    if (!matches || matches.length < 5) {
        console.error(`Failed to match S3_LIVE_CLASS_KEY_REGEX on item ${item.Key}`);
        return;
    }
