...
public class Handler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
	...
	Boolean ENV = System.getenv("ENV").equalsIgnoreCase("beta");
	...
	MongoCollection<Document> RRcollection = ENV ? database.getCollection("rr-tournaments-beta") : database.getCollection("rr-tournaments ");

	MongoCollection<Document> RRplayercollection = ENV ? database.getCollection("rr-players-beta") : database.getCollection("rr-players");
		...
		switch (queryParams.get("mode")){
			case "register" -> {
				...
				try{
					...
					res = handler.playerRegister(discordid,
