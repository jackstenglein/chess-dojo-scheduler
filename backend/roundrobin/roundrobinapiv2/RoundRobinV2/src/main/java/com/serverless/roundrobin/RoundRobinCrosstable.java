...
public class RoundRobinCrosstable {
    ...
    public void updateCrossTableScores(String player1, String player2, GameState state, Platform platform){
        ...
        player1Discord = actions.performGeneralSearch(this.RRplayercollection, platform.getPlayerField(), player1).getString("Discordname");
        player2Discord = actions.performGeneralSearch(this.RRplayercollection, platform.getPlayerField(), player2).getString("Discordname");
    ...
    public void withdrawPlayer(int currentSize, int playerIndex) throws RoundRobinException {
        ...
        for(ArrayList<String> sublists: actualList){
            sublists.remove(playerIndex);
        }
