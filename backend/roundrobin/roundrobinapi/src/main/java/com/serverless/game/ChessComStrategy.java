...
public class ChessComStrategy implements CalculateResultStrategy {
    ...
    public ChessComStrategy(String gameURL, String player, MongoCollection<Document> rRplayercollection, MongoCollection<Document> rRcollection, String tournamentID){
    ...
    public void calculateGameResult() throws RoundRobinException {
        ...
        try {
            List<ArchiveGame> list = client.getMonthlyArchiveForPlayer(player, year, month).getGames().stream().filter(s -> s.getUrl().equalsIgnoreCase(gameURL)).toList();
