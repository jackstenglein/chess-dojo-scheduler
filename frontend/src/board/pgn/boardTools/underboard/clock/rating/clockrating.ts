...
export function calculateTimeRating(
    ...
    if (dataset.length < 10) {
        console.warn(
            `[TimeRating] Dataset too small (${dataset.length} moves), returning default rating`,
        );
        return 1500;
    }
    ...
    const { slope: rawPlayerSlope, intercept: rawPlayerIntercept } = fitLine(
        dataset,
        openingSize,
        endMoveIndex,
    );
    ...
    if (BEST_REF_MOVE_SIZE[totalTimeKey]) {
        ...
        normalizationFactor = Math.min(
            2,
            Math.max(0.5, BEST_REF_MOVE_SIZE[totalTimeKey] / Math.max(10, actualMoveCount)),
        );
    }
    ...
    console.log(
        `Game length: ${dataset.length} moves, Using moves ${openingSize} to ${endMoveIndex}`,
    );
    ...
    console.log(
        `Best Intercept: ${bestIntercept.toFixed(2)}, Player Intercept: ${playerIntercept.toFixed(2)}`,
    );
    ...
    if (Math.abs(playerSlope) > Math.abs(bestSlope)) {
        slopeDeviation = Math.abs(
            (bestSlope - playerSlope) / Math.max(slopeEpsilon, Math.abs(playerSlope)),
        );
        interceptDeviation = Math.abs(
            (playerIntercept - bestIntercept) / Math.max(interceptEpsilon, Math.abs(bestIntercept)),
        );
    } else {
        slopeDeviation = Math.abs(
            (playerSlope - bestSlope) / Math.max(slopeEpsilon, Math.abs(bestSlope)),
        );
        interceptDeviation = Math.abs(
            (playerIntercept - bestIntercept) / Math.max(interceptEpsilon, Math.abs(bestIntercept)),
        );
    ...
    console.log(
        `[TimeRating] Total deviation: ${totalDeviation.toFixed(3)}, Adjusted: ${adjustedDeviation.toFixed(3)}`,
    );
