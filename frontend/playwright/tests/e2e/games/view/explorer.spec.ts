...
test.describe('Position Explorer', () => {
    test.beforeEach(async ({ page }) => {
        await page.goto(
            '/games/1500-1600/2024.07.24_3a1711cf-5adb-44df-b97f-e2a6907f8842',
        );
    });
    ...
    test('opens to dojo tab by default', async ({ page }) => {
        ...
        await expect(
            page.getByRole('tab', { name: 'Dojo', selected: true }),
        ).toBeVisible();
    });
    ...
    test('opens other tabs', async ({ page }) => {
        ...
        await expect(
            page.getByRole('tab', { name: 'Masters', selected: true }),
        ).toBeVisible();

        await getBySel(page, 'explorer-tab-button-lichess').click();
        await expect(
            page.getByRole('tab', { name: 'Lichess', selected: true }),
        ).toBeVisible();

        await getBySel(page, 'explorer-tab-button-tablebase').click();
        await expect(
            page.getByRole('tab', { name: 'Tablebase', selected: true }),
        ).toBeVisible();
    });
